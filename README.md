# Aufgabenstellung

💻 Alternative Klassenarbeit – Projekt „Meine eigene Website“


📝 Aufgabenstellung

Anstelle einer Klassenarbeit erstellt ihr eine eigene Website mit HTML.
Ziel ist es, zu zeigen, dass ihr die Inhalte des Kapitels HTML sicher anwenden könnt.

Eure Website muss alle folgenden Inhalte enthalten und korrekt umsetzen:



🔹 Pflichtbestandteile (alles erledigt)

- [x] 1️⃣ Grundstruktur einer HTML-Seite
  - [x] `<!DOCTYPE html>`
  - [x] `<html>, <head>, <body>`
  - [x] Titel der Seite (`<title>`)
  - [x] sinnvolle Überschriften (`<h1>–<h3>`)
  - [x] Absätze (`<p>`)

- [x] 2️⃣ Guter HTML-Stil
  - [x] saubere Einrückungen
  - [x] sinnvolle Strukturierung
  - [x] sprechende Dateinamen
  - [x] Kommentare im Code (<!-- Kommentar -->)
  - [x] keine unnötigen Tags

- [x] 3️⃣ Bilder einbinden
  - [x] mindestens 2 Bilder
  - [x] Verwendung des `<img>`-Tags
  - [x] sinnvolle alt-Texte
  - [x] relative Pfadangaben
  - [x] passende Bildgröße

- [x] 4️⃣ Listen und Zeichencodierung
  - [x] mindestens:
    - [x] eine ungeordnete Liste (`<ul>`)
    - [x] eine geordnete Liste (`<ol>`)
  - [x] mindestens ein Sonderzeichen (z. B. € © ä ö ü ß)
  - [x] korrekte Zeichencodierung (UTF-8 im `<meta>`-Tag)

- [x] 5️⃣ Links im World Wide Web
  - [x] ein externer Link (z. B. zu einer bekannten Website)
  - [x] ein interner Link (z. B. zu einer zweiten eigenen HTML-Seite)
  - [x] sinnvoller Linktext (nicht nur „hier klicken“)

- [x] 6️⃣ Tabellen
  - [x] mindestens eine Tabelle mit:
    - [x] Tabellenüberschrift (`<th>`)
    - [x] mindestens 3 Zeilen
    - [x] mindestens 2 Spalten
  - [x] sinnvolle Inhalte

- [x] 7️⃣ Navigation (für eine 1 notwendig)
  - [x] Eine einfache Navigationsleiste
  - [x] Verlinkung zwischen mindestens 2 HTML-Seiten
  - [x] Navigation auf jeder Seite sichtbar


📁 Verwendete Dateien in `docs/`

- `docs/index.html`
- `docs/creator.html`
- `docs/install.html`
- `docs/mods.html`
- `docs/more.html`
- `docs/style.css`
- `docs/img/`


🎯 Thema der Website

Das Thema ist frei wählbar. Beispiele:

	• Mein Hobby
	• Meine Traumreise
	• Mein Lieblingsspiel
	• Eine Sportart
	• Ein Tier
	• Eine Band
	• Ein Film
	• Technik / Gaming
	• Schulprojekt

Wichtig: Das Thema soll sinnvoll strukturiert präsentiert werden.



📦 Abgabe

Abzugeben sind:

	• Alle HTML-Dateien
	• Ein Ordner mit Bildern
	• Die Dateien müssen lokal im Browser funktionieren
	• Protokoll



📊 Bewertungskriterien (allgemein)

	• Technische Umsetzung aller Pflichtbestandteile
	• Sauberer Code
	• Funktionierende Links
	• Struktur & Übersichtlichkeit
	• Kreativität & Gestaltung
	• Eigenständigkeit
